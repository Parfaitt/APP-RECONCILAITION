...
class IfuturPayoutProcessor:
    ...
    def process(self):
        ...
        #correspondance_idoperator = dfop_unique.set_index('REFERENCE')['REFERENCE']
        correspondance_operator = dfop_unique.set_index('REFERENCE')['CANAL']
        ...
        dfpmt['OPERATOR'] = dfpmt['ID Opérateur'].map(correspondance_operator)
        ...
        #NBSI PMT &CINETPAY
        dfpmt['IFUTUR'] = dfpmt['ID Opérateur'].isin(ts['REFERENCE']).astype(int)
        ts['PMT'] = ts['REFERENCE'].isin(dfpmt['ID Opérateur']).astype(int)
